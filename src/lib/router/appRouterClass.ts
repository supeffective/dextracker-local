import { createPatternRegex, getWindowHash, parsePathParams, parseUri } from './parser'
import type { CurrentRoute, PageComponent, Route } from './types'

export class AppRouter implements Iterable<Route> {
  [Symbol.iterator](): Iterator<Route> {
    return this.routes[Symbol.iterator]()
  }
  public static readonly Hasbang = '/#/'
  private routes: Route[] = []
  private fallback?: PageComponent
  ...
  refresh(path: string): void {
    ...
    const queryEntries = Array.from(parsedPath.query.entries())
  ...
  register(onUpdate?: (router: AppRouter) => void): () => void {
  ...
  add(pattern: string, component: PageComponent): void {
  ...
  setFallback(component: PageComponent): void {
    this.fallback = component
  }

  getFallback(): PageComponent | undefined {
